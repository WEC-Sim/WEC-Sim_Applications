...
v_cutin = 4;                                % Cut in speed [m/s]
v_rated_try=10.4;                           % Nominal rated wind speed [m/s]
v_cutout = 25;                              % Cut out speed [m/s]
v_discr = 0.5;                              % Wind speed discretisation [m/s]
vw_R2in_try=6;                              % Boundary wind speed between region 1.5 and 2 (only used for ROSCO controller) [m/s]
calc_vw_R2in_flag=0;                        % Method to calculate vw_R2in_try (1: exact throught optimization, 0:approximated)
omega_min =3*pi/30;                         % Minimun rotor speed (only used for ROSCO controller) [rad/s]
omega_rated_try=7.56*pi/30;                 % Nominal rotor rated speed [m/s]
max_Thrust_factor=0.85;                     % Maximum thrust force factor (max thrust with peak shaving/max thrust without peak shaving)
Prated=10e6;                                % Rated power [W]
...
%% SAVE
save('SteadyStates_10MW','SteadyStates')
