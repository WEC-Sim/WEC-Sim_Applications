...
if 1
%% Cgen control
torqueMaxRate=2e+6;       % Max generator torque rate (Nm/s) 

%% Pitch control
thetaMaxRate=deg2rad(8);  % Max blade pitch rate (rad/s)
%% Filters
...
Ctrl.YawControl.Kp=0.1; 
Ctrl.YawControl.Ki=0.002;
Ctrl.YawControl.Kd=0;
Ctrl.YawControl.tau_system=10;
end
%% Linearization
delta_rel=1e-6;
%% BEM Settings
...
load('SteadyStates_NREL5MW.mat'); % Load steady states data 
%% BEM data
...
BEM_data.WT_r_hub_ws=[WTcomponents.hub.overhang*cosd(WTcomponents.nacelle.tiltangle);
                                                    0;
...
Ctrl.Baseline.torqueMaxRate=torqueMaxRate;
