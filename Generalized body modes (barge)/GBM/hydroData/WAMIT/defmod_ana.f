...
C-----------------------------------------------------------------------
      ...
      REAL KL(1:4)
C-----------------------------------------------------------------------
C   The following example defines 4 new modes corresponding to
...
C-----------------------------------------------------------------------
      NEWMDS=4
      KL(1)=4.73004074486270
      KL(2)=7.85320462409584
      KL(3)=10.9956078380017
      KL(4)=14.1371654912575
      ...
      DO J=7,NEWMDS+6
        ...
        V(J)=ZERO
        N=J-6
        SN=(COSH(KL(N))-COS(KL(N)))/(SINH(KL(N))-SIN(KL(N)))
        WL=COSH(KL(N))+COS(KL(N))-SN*(SINH(KL(N))+SIN(KL(N)))
        W(J)=COSH(KL(N)*Q)+COS(KL(N)*Q)-SN*(SINH(KL(N)*Q)+SIN(KL(N)*Q))
        W(J)=W(J)/WL
      END DO
