...
body(2).initial.displacement = [0 0 -.21];     % Initial Displacement

%% PTO and Constraint Parameters
% Floating (3DOF) Joint
constraint(1) = constraintClass('Constraint1'); 
constraint(1).location = [0 0 0];    
constraint(1).initial.displacement = [0 0 -.21];     % Initial Displacement   
...
% mooring(2).location = [0 0 -10];
% mooring(2).initial.displacement = [0 0 0];  % Initial Displacement

% mooring(3) = mooringClass('mooring2');           % Initialize mooringClass
% mooring(3).moorDyn = 1;
% mooring(3).moorDynLines = 2;                    % Specify number of lines
% mooring(3).moorDynNodes(1:3) = 6;              % Specify number of nodes per line
% mooring(3).moorDynNodes(4:6) = 16;              % Specify number of nodes per line
% mooring(3).location = [0 0 -20];
